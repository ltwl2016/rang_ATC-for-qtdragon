...
class HandlerClass:
    def __init__(self, halcomp, widgets, paths):
        ...
        self.onoff_list = ["frame_program", "frame_tool", "frame_dro", "frame_override", "frame_status","frame_02","btn_setup_pocket"]
        ...
        self.button_response_list = ["btn_start", "btn_home_all", "btn_home_x", "btn_home_y",
                            ...
                            "macrobutton7", "macrobutton8", "macrobutton9","macrobutton10","macrobutton11",
                            "macrobutton12","macrobutton13","macrobutton14","btn_setup_pocket"]
...
""" %(  os.path.expanduser('~/linuxcnc'),
    ...
    def initialized__(self):
        ...
        self.w.btn_setup_pocket.clicked.connect(self.btn_pocket_clicked)
        #self.w.lbl_estop_image.click_event.connect(self.enable_onoff)
    ...
    def init_pins(self):
        ...
        pin = QHAL.newpin("toolchange-number", QHAL.HAL_S32, QHAL.HAL_IN)
        pin.value_changed.connect(self.toolchange_changed)
...
#                    QtCore.Qt.Key_F3,QtCore.Qt.Key_F4,QtCore.Qt.Key_F5):
    ...
    def percent_done_changed(self, fraction):
        ...
        if fraction <0:
            ...
            self.w.progressBar.setFormat('完成率: {}%'.format(fraction))

    def all_homed(self, obj):
        self.home_all = True
        self.w.btn_home_all.setText("全部机床原点已回")
        if self.first_turnon is True:
            ...
            if self.w.chk_reload_tool.isChecked():
                ...
                #if (self.reload_tool <= 5) and (self.reload_tool >0):
                    #button = self.w["macrobutton{}".format(self.reload_tool+3)]
    ...
    def not_all_homed(self, obj, list):
        self.home_all = False
        self.w.btn_home_all.setText("全部轴回机床原点")

    def hard_limit_tripped(self, obj, tripped, list_of_tripped):
        self.add_status("硬限位触发", CRITICAL)
    ...
    def toolchange_changed(self, data):
        for n in range(1,6):
            self.h['led_{}'.format(n)] = False
        if data in range(1,6):
            button = self.w["macrobutton{}".format(int(data)+3)]            
            button.setChecked(True)
            self.h['led_{}'.format(data)] = True
        if data == 0:
            self.w["macrobutton3"].setChecked(True)
        if data >= 6:     
            self.w["macrobutton2"].setChecked(True)
    ...
    """def toolchange_buttonGroup_clicked(self):
        tl = self.reload_tool
        if tl <= 5 and tl > 0:
            button = self.w['macrobutton{}'.format(tl)]
            button.setChecked(True)
    """   
    ...
    def btn_home_all_clicked(self, obj):
        ...
        # instantiate dialog box
            info = "回退到未回机床原点状态?"
            mess = {'NAME':'MESSAGE', 'ID':'_unhome_', 'MESSAGE':'回退原点', 'MORE':info, 'TYPE':'OKCANCEL'}
            ACTION.CALL_DIALOG(mess)
    ...
    def btn_copy_file_clicked(self):
        ...
        if source[1] is False:
            self.add_status("特殊资源并非文件", WARNING)
            return
    ...
    def btn_m61_clicked(self):
        ...
        if len(checked) > 1:
            self.add_status("只能选择一把刀具", WARNING)
        elif checked:
            self.add_status("加载刀具{}".format(checked[0]))
            ACTION.CALL_MDI("M61 Q{} G43".format(checked[0]))
        else:
            self.add_status("无选择的刀具", CRITICAL)

    def btn_touchoff_clicked(self):
        if STATUS.get_current_tool() == 0:
            self.add_status("无法探测因为没有选择刀具", CRITICAL)
            return
        if not STATUS.is_all_homed():
            self.add_status("必须回机床原点后才能探测", CRITICAL)
            return
        # instantiate dialog box
        sensor = self.w.sender().property('sensor')
        unit = "mm" if INFO.MACHINE_IS_METRIC else "in"
        info = "确保刀具位于{} {}范围内能触发刀具传感器".format(self.w.lineEdit_max_probe.text(),unit)
    ...
    def btn_save_status_clicked(self):
        ...
        self.add_status("保存状态文件为{}".format(filename))
    ...
    def chk_run_from_line_checked(self, state):
        self.w.btn_start.setText("始于\n1") if state else self.w.btn_start.setText("循环\n开始")
    ...
    def load_code(self, fname):
        ...
        if not file_extension in (".html", '.pdf'):
            if not (INFO.program_extension_valid(fname)):
                self.add_status("无效或未知的文件扩展名{}".format(file_extension), CRITICAL)
                return
            self.w.cmb_gcode_history.addItem(fname)
            self.w.cmb_gcode_history.setCurrentIndex(self.w.cmb_gcode_history.count() - 1)
            self.w.cmb_gcode_history.setToolTip(fname)
            ACTION.OPEN_PROGRAM(fname)
            self.add_status("加载程序文件: {}".format(fname))
            self.adjust_stacked_widgets(TAB_MAIN)
            ...
            try:
                if os.path.exists(fname):
                    ...
                    self.add_status("加载HTML文件: {}".format(fname), CRITICAL)
                else:
                    self.w.webwidget.setHtml(self.html)
            except Exception as e:
                self.add_status("错误加载HTML文件{} : {}".format(fname,e))
            # look for PDF setup files
            # load it with system program
            fname = filename+'.pdf'
            if os.path.exists(fname):
                self.PDFView.loadView(fname)
                self.add_status("加载PDF文件: {}".format(fname))
            else:
                self.PDFView.loadSample('setup_tab')
        ...
        if file_extension == ".html":
            try:
                self.w.webwidget.loadFile(fname)
                self.add_status("加载HTML文件 : {}".format(fname))
...
<br>'% (property_names.get(i), props[i])
    ...
    def touchoff_return(self, data):
        self.add_status("探测器成功探测并返回")
        self.add_status("探测器返回数据: "+data, CRITICAL)
    ...
    def kb_jog(self, state, joint, direction, fast = False, linear = True):
        ...
        if not STATUS.is_man_mode() or not STATUS.machine_is_on():
            self.add_status('机床必须处于手动模式才能使用', CRITICAL)
            return
        if linear:
    ...
    def enable_onoff(self, state):
        if state:
            self.add_status("机床开启")
        else:
            self.add_status("机床关闭")
        self.w.btn_spindle_pause.setChecked(False)
        self.h['eoffset-spindle-count'] = 0
    ...
    def use_keyboard(self):
        if self.w.chk_use_keyboard.isChecked():
            ...
            self.add_status('键盘设置已关闭')
            return False

    def do_file_copy(self):
        try:
            copyfile(self.source_file, self.destination_file)
            self.add_status("拷贝文件从{}到{}".format(self.source_file, self.destination_file))
        except Exception as e:
    ...
    def stop_timer(self):
        ...
        if STATUS.is_auto_mode():
            self.add_status("运行时停止at {}".format(self.w.lbl_runtime.text()))
    ...
    def adjust_stacked_widgets(self,requestedIndex,mode_change=False):
        ...
        if main_index != requestedIndex and not main_index in(TAB_CAMERA,TAB_GCODES,TAB_SETUP):
            self.add_status("自动模式时无法切换其他功能页面", WARNING)
